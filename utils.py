...
from transforms.classification.data_transforms import MEAN, STD
    ...
    def setupSegNet(modelType,scale):
        ...
        if modelType == 'espnetv2':
            model = espNetModel(segClasses,scale,modelWeightsFile)
        elif modelType == 'dicenet':
            model = diceNet(modelWeightsFile, segClasses)
        return model


    def espNetModel(classes,scale,weights,dataset="city"):
